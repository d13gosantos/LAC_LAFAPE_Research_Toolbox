function [obsSysOut,Lsigma,Rsigma,params,lmiMat] = uncertfdiocontinuos2(obsSys,eta,omegazero,alpha)
    ...
    theta_x = arrayfun(@(x) sdpvar(1,1),1:M,'UniformOutput',false);
    
    cellfun(@(var) assign(var,lmitol),theta_a,'UniformOutput',false);
    cellfun(@(var) assign(var,lmitol),theta_x,'UniformOutput',false);
    ...
    RestrH = arrayfun(@(i) [Hsigma{1,i}>=lmitol*eye(p),theta_a{i}>=lmitol,theta_x{i}>=lmitol],1:M,'UniformOutput',false);
   ...
   Restr = [RestrMat,RestrSminus,RestrLinfty]; %Necessary for dwell time restrictions
    ...
    function params = get_params(feasible)
        ...
        if feasible
            ...
            params.theta_x = cellfun(@value,theta_x,'UniformOutput',false);
            Nsquare = cellfun(@(N) N'*N,Na,'UniformOutput',false);
            Nxesquare = cellfun(@(N) xe'*N'*N*xe,Na,'UniformOutput',false);
            theta_a_bar = max([params.theta_x{:}])*eigmax(Nsquare);
            theta_x_bar = max([params.theta_x{:}])*eigmax(Nxesquare);
    
            params.theta_a_bar = theta_a_bar;
            params.theta_x_bar = theta_x_bar;
            [jth,jthmax] = getthreshold(params.gamma,dmax,theta_a_bar,theta_x_bar,omegazero,alpha);
    ...
    function [lmiMat,constraints,objective] = LinftyUncert()
        ...
        for j=1:M
            ...
            s23 = zeros(cEf,cMa);
            s24 = zeros(cEf,cMa);
            s33 = -theta_x{j}*eye(cMa);
            s34 = zeros(cMa,cMa);
            s44 = -theta_x{j}*eye(cMa);
            S{j} = [s11 s12 s13 s14;
                s12' s22 s23 s24;
                s13' s23' s33 s34;
                s14' s24' s34' s44];
        ...
        K =  sdpvar(rK,cK);
        for j =1:M
            E{j} = (eta-gamma*dmax^2-theta_x{j}*eigmax(Na{j}'*Na{j})*omegazero^2-theta_x{j}*xe'*Na{j}'*Na{j}*xe);
        end
        %         for j =1:M
