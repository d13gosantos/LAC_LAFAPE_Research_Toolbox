function [optparams,vectors,obsControlOpt] = optimizecontrol(obsSys,vartheta,etamax,etasteps,czeta,omegainfty)
   ...
   % kmax = length(etaVector);
    ...
    obsControl{kmax} = [];
    for k = 1:kmax
        obsControl{k} = robftslgains(obsSys,etaVector(k),vartheta,czeta,1);
